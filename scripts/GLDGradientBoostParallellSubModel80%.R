...
confident_preds$FiveDayClose <- lead(confident_preds$Close, 5)
confident_preds <- na.omit(confident_preds)
confident_preds$priceDiff <- confident_preds$Close - confident_preds$FiveDayClose
confident_preds$profit <- ifelse(confident_preds$correct, abs(confident_preds$priceDiff), -abs(confident_preds$priceDiff))

# ======================
# 🔮 Live Prediction Using Latest EOD Data
# ======================
if(test == FALSE){
  live_i <- nrow(df)
  
  # Build training window
  df_window <- df[(live_i - lookback_window):(live_i - 1), ] %>%
    mutate(
      next_close = lead(Close, horizon),
      pct_change = (next_close - Close) / Close * 100,
      target = case_when(
  ...
  # Add neutral prob to training set
  df_window$neutral_prob <- predict(neutral_model, newdata = df_window, type = "response")
  ...
  dtrain <- xgb.DMatrix(
    data = as.matrix(df_train[, feature_cols]),
    label = df_train$target_numeric,
    weight = ifelse(df_train$target_numeric == 0, 7, 1)
  )
  ...
  # Prepare today's prediction row
  row <- df[live_i, ] %>%
    mutate(
      Return_1D = (Close / df$Close[live_i - 1]) - 1,
      Return_3D = (Close / df$Close[live_i - 3]) - 1,
      Return_5D = (Close / df$Close[live_i - 5]) - 1,
      HiLo = High / Low,
      OpenClose = Open / Close
    )
  ...
  cat("Coverage:", round(coverage * 100, 2), "% of all predictions\n")
  print(table(predicted = confident_results$Prediction, actual = confident_results$Actual))
  cat("\n🔮 LIVE PREDICTION —", df$Date[live_i], "\n")
  cat("Prediction:", pred_class, "\n")
  cat("Confidence:", round(max_prob, 4), "\n")
  cat("Neutral probability:", round(row$neutral_prob, 4), "\n")
  cat(ifelse((max_prob>=threshold & row$neutral_prob<= .5), "\u2705 Trade advised today\n", "\u274C No trade advised today\n"))
}
